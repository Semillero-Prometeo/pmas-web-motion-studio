    ...
    def run(self):
        while self.running:
            if self.queue and self.arduino and self.arduino.is_open:
                ...
                # Descartar comandos si no hay conexión
                self.queue.popleft()
            time.sleep(0.002)
...
class Bloque(QGraphicsRectItem):
    ...
    def mouseReleaseEvent(self, event):
        ...
        if scene and hasattr(scene, 'parent_ui'):
            ...
            fila_destino = max(0, min(fila_destino, len(ui.bloques) - 1))

            bloques_ordenados = sorted(ui.bloques, key=lambda b: (b.pca, b.servo))
            if fila_destino < len(bloques_ordenados):
                bloque_destino = bloques_ordenados[fila_destino]
                if bloque_destino is not self:
